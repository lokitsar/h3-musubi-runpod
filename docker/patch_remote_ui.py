...
replacement = """async function api(path, options = {}) {
  // RunPod is headless. Replace native Windows/Tk path dialogs with a browser
  // prompt for paths that exist inside the Pod.
  if (path === "/api/path/select") {
    let body = {};
    try { body = JSON.parse(options.body || "{}"); } catch (_) {}
    const fallback = body.initial || "/workspace";
...
print(f"Patched headless path selection in {path}")
